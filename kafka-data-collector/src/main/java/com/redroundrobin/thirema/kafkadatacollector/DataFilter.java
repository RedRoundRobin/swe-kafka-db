  ...
  private List<Message> filterRealAlerts(List<JsonObject> data) throws SQLException {
    ...
    for (JsonObject device : data) {
      ...
      for (JsonElement jsonSensor : device.get("sensors").getAsJsonArray()) {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM alerts WHERE (sensor_id = ? AND deleted = false AND ((last_sent < (clock_timestamp() - INTERVAL '30 seconds')) OR (last_sent IS NULL))) AND ((type = 0 AND ? > threshold) OR (type = 1 AND ? < threshold) OR (type = 2 AND ? = threshold))")) {
